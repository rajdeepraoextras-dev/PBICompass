...
- plain_english: the business definition, 1-2 sentences. State what the number means, why the calculation or exclusion matters when interpreting it, and the decision or comparison it can safely support when that follows from the supplied context. Do not describe DAX syntax here. For a hardcoded assumption, explain that it makes the result a modeled estimate and how that limits interpretation; do not stop at "business meaning requires confirmation."
- calculation_logic: 1-2 sentences on how it is computed, in plain terms. Explain why each material denominator, exclusion, time window, or hardcoded assumption is used or how it changes interpretation (e.g. "Sums Sale_Value over Orders and restricts rows to the selected year so comparisons use the same reporting period."). Do not repeat the business definition.
