...
- **A2-2 — Render duplication (MEDIUM).** `render/html.py` carries its own copy of the shell that `render/_html_shell.py` also provides. Every presentation change is done twice. Migrate `html.py` onto the shared shell (golden tests first).
...
- **6.1 Migrate `html.py` onto `_html_shell.py`** (A2-2) — prerequisite for consistent polish; do it behind golden tests. (0.5 day)
...
**Day 11 (Jul 22) — `html.py` → `_html_shell.py` migration (A2-2).** Golden snapshots first, then delete the duplicated shell. _Done-when:_ snapshots byte-identical (or intentional-diff reviewed); duplication gone.
...
**Day 13 (Jul 24) — Wireframe v2 (part 2) + reintroduce.** Resolve field links; uncomment `html.py:456-457`, `user_guide.py:146-147`; href-resolution golden test. _Done-when:_ every wireframe `href` resolves; wireframes visible again.

**Day 14 (Jul 27) — Lineage graph redesign + reintroduce.** Readable layered SVG, dark-mode-aware; uncomment `html.py:344-345`; keep edge-list fallback. _Done-when:_ lineage renders cleanly; md/docx fallback intact.
...
**Day 16 (Jul 29) — Persistent JobStore.** Back `JobStore` with the DB behind its existing method surface; outputs as BLOB/object with TTL sweep (A2-1). _Done-when:_ single-instance restart survives in-flight jobs; zero-retention test still passes.

**Day 17 (Jul 30) — Managed Postgres for accounts.** Port `AccountStore` to Postgres behind the same interface; `PBICOMPASS_DB` URL scheme selects backend; SQLite stays for self-host. _Done-when:_ accounts/keys/quotas survive redeploy; both backends tested.

**Day 18 (Jul 31) — Async worker (Celery+Redis or managed).** Move `process_job` onto the queue; remove the Cloud Run CPU-throttling failure class. _Done-when:_ jobs complete regardless of request-driven CPU windows; watchdog still bounds stalls.

**Day 19 (Aug 3) — Observability.** JSON structured logging with request/job IDs (content-free, asserted in test); Sentry with PII scrub; real readiness `/healthz` (DB+queue). _Done-when:_ a failed job produces a traceable, content-free log + Sentry event.

**Day 20 (Aug 4) — Metrics, rate limiting, secrets, backups.** Jobs/min, failure rate, cost/job, 429 rate; per-IP rate limits on upload; secrets to platform store; Postgres backups + restore drill. _Done-when:_ dashboards live; restore drill passes; no secret in image/env.
...
**Day 21 (Aug 5) — User model + password auth.** `users` table, argon2/bcrypt hashing, `user↔account` membership; signup/login endpoints; sessions (HTTP-only/Secure/SameSite, CSRF). _Done-when:_ a new user can register and log in; API-key path unchanged.

**Day 22 (Aug 6) — Email flows.** Transactional provider wired; email verification + password reset; rate-limited auth routes (reuse lockout pattern). _Done-when:_ verify + reset work end-to-end; unverified users gated appropriately.

**Day 23 (Aug 7) — "Sign in with Microsoft" (OIDC).** Entra ID OAuth2/OIDC alongside password (§7.3); the enterprise-SSO stepping stone. _Done-when:_ a Microsoft account can sign in and map to a user.

**Day 24 (Aug 10) — Account dashboard.** `/app`: plan + usage vs quota, API-key create/revoke UI, job history (status only). _Done-when:_ a user self-serves keys and sees usage without the admin token.
